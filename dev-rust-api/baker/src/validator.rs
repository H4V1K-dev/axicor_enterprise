...
pub fn validate_physics_and_anatomy(
    ...
    let height_sum: f32 = layers.iter().map(|l| l.height_pct).sum();
...
mod tests {
    use super::*;
    use config::{LayerConfig, NeuronTypeDistribution, SimulationParams};
    ...
    fn test_valid_params_pass() {
        let result = validate_physics_and_anatomy(&valid_sim(), &valid_layers());
        assert!(result.is_ok());
    }
    ...
    fn test_invalid_height_sum() {
        ...
        layers[0].height_pct = 0.3; // sum = 0.9 ≠ 1.0
        let result = validate_physics_and_anatomy(&valid_sim(), &layers);
    ...
    fn test_invalid_composition_share() {
        let mut layers = valid_layers();
        layers[0].composition[0].share = 0.5; // sum = 0.5 ≠ 1.0
        let result = validate_physics_and_anatomy(&valid_sim(), &layers);
    ...
    fn test_invalid_signal_speed_fractional() {
        ...
        let result = validate_physics_and_anatomy(&sim, &valid_layers());
    ...
    fn test_fail_fast_height_before_composition() {
        ...
        let mut layers = valid_layers();
        layers[0].height_pct = 0.1; // height sum = 0.7
        layers[0].composition[0].share = 0.5; // composition also invalid
        let result = validate_physics_and_anatomy(&valid_sim(), &layers);
        assert!(matches!(result, Err(BakerError::InvalidLayerHeights { .. })));
    }
}
