...
mod tests {
    ...
    #[test]
    fn test_hip_invalid_handle_checks() {
        let backend = HipBackend::new(0).unwrap();
    ...
    #[test]
    fn test_hip_compute_capability_check() {
        #[cfg(feature = "mock-gpu")]
        {
            use mock::COMPUTE_CAPABILITY_MAJOR;
            
            // Set compute capability below CDNA gfx90a (9.0)
            COMPUTE_CAPABILITY_MAJOR.with(|f| f.store(8, Ordering::Relaxed));
            let res = HipBackend::new(0);
            COMPUTE_CAPABILITY_MAJOR.with(|f| f.store(9, Ordering::Relaxed)); // restore default
    ...
    #[test]
    fn test_hip_launch_parameter_error() {
        #[cfg(feature = "mock-gpu")]
        {
            use compute_api::GpuBackend;
            let backend = HipBackend::new(0).unwrap();
    ...
    #[test]
    fn test_hip_ghost_capacity_overflow() {
        use compute_api::GpuBackend;
        let backend = HipBackend::new(0).unwrap();
        let layout = ShardLayout {
            padded_n: 64,
    ...
    #[test]
    fn test_hip_pinned_ram_exhaustion() {
        #[cfg(feature = "mock-gpu")]
        {
            use compute_api::GpuBackend;
    ...
    #[test]
    fn test_hip_compile_assertions() {
        ...
        // Check that HIP source files contain core_math.h (INV-COMPUTE-HIP-003 validation)
        let hip_path = std::path::Path::new("src/hip/physics.hip");
        if hip_path.exists() {
            let content = std::fs::read_to_string(hip_path).unwrap();
            assert!(
                content.contains("#include \"core_math.h\""),
                "physics.hip must include core_math.h"
            );
        }
    }
}
