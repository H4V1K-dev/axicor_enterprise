...
## 6. Владение и хранение кэша (Cache Ownership & Intermediate Checkpoints)

- **Производный статус**: Все объекты предпросмотра являются derived-данными. Они не мутируют Store напрямую и **никогда не записываются** в канонический TOML.
- **Локальный кэш проекта**: Сессионные предпросмотры и промежуточные бинарные сэмплы сохраняются в каноническом дисковом каталоге проекта `.local-storage/artifacts/` под управлением [Artifact Cache Registry](artifact-cache-registry-spec-ru.md).
- **Промежуточные состояния и контрольные точки (Preview Checkpoints)**: Для тяжелых предпросмотров (генерация тысяч сом, морфогенез аксонов) конвейер поддерживает сохранение промежуточных контрольных точек (*checkpoints*). Они используются для отладки (debugging), повторного воспроизведения (replay) и точек восстановления. Чекпоинты подчиняются правилам авто-очистки (*auto-evict policy*) и не раздувают проект.
- **Инвариант безотказности**: Очистка кэша предпросмотров не приводит к потере данных модели, а лишь требует повторного запроса к AxiEngine.
...
|---|---|---|
| 2026-06-28 | 0.2.0 | Добавлена поддержка промежуточных состояний предпросмотра и контрольных точек (Preview Checkpoints) в каноническом пути `.local-storage/artifacts/` под управлением Artifact Registry для отладки и восстановления без засорения TOML. |
