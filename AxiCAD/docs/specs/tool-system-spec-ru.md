...
2. **Transform Tools**:
   - Инструменты перемещения (`MoveTool`) и изменения габаритов (`ResizeTool`) для шардов и департаментов; вращение шардов дискретно на 90° вокруг оси Z осуществляется через быстрый интент/команду, а полноценный 3-осевой `RotateTool` и вращение департаментов вынесены в future extension.
3. **Composition Tools**:
...
| :--- | :--- | :--- |
| 2026-06-28 | v0.5.0 | Проведена чистка структуры документа: вычищен дублирующий старый блок Open Decisions #1-5, восстановлена целостность раздела 8 (Preview & Staging / Constraint Preflight), упорядочена сквозная нумерация разделов 9-16, синхронизированы категории инструментов с MVP rotation policy (дискретные 90° вокруг Z для шардов, без вращения департаментов). |
| 2026-06-28 | v0.4.0 | Синхронизированы канонические kebab-case интенты ввода (`add-to-selection`, `axis-lock`, `confirm` и др.), оформлен раздел зафиксированных решений 15 (Tool Registry, Presets, Modal Tools, suspend/resume, Keymaps) и оставлен единый открытый вопрос по автороутингу в разделе 16. (Закрыты Open Decisions #36-40). |
