...
3. **Где хранить настройки пресетов камер (Camera Presets)?**
   * *Контекст*: Кнопки быстрого переключения камеры (например, "Вид сверху", "Фокус на Sensory"). Должны ли эти пресети быть зашиты в код редактора или настраиваться в JSON проекта?
