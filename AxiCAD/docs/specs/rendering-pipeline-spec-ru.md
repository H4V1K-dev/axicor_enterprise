...
*На текущий момент все открытые вопросы графического конвейера рендеринга разрешены.*
...
| :--- | :--- | :--- |
| 2026-06-28 | v0.5.0 | Зафиксированы архитектурные решения: утвержден вектор на Rust/WGPU с исследованием готовых фреймворков (в т.ч. минимальных модулей Bevy), зафиксирована политика focus attenuation и semantic opacity tiers для прозрачности, определено сохранение пресетов камер в project metadata, подтвержден статус GPU picking buffer как графической оптимизации и определен spatial HUD для 3D-меток. (Закрыты Open Decisions #1-5). |
