...
/** Концептуальный дескриптор экземпляра вьюпорта (Multi-Viewport Instance Handle) */
export type ViewportInstanceId = string;

export interface RenderSurfaceHandle {
  readonly surfaceId: ViewportInstanceId;
...
- **Departments (Департаменты)**: Отображаются в MVP как полупрозрачные параллелепипеды, выравниваемые по сетке (AABB / grid-aligned transparent boxes), с тонкими светящимися ребрами. Внутренние шарды должны быть четко видны сквозь департамент.
...
### 16.4 Мульти-вьюпортная архитектура рендеринга (Multi-Viewport Architecture)
- **Экземпляры вьюпортов (Viewport Instances)**: Конвейер рендеринга спроектирован с поддержкой одновременной отрисовки нескольких независимых панелей/окон просмотра (`Viewport Instances`) на базе одного графа сцены (`RenderSceneModel`).
- **Изоляция камер и проекций**: Каждый экземпляр вьюпорта имеет собственный `ViewportCameraState`, режим проекции (перспектива / ортогональная), параметры масштаба и независимый `focus state`.

---

## 17. Зафиксированные архитектурные решения (Resolved Architecture Decisions)

В рамках утверждения архитектуры визуального слоя зафиксированы следующие ключевые решения:

1. **Целевое направление бэкенда (Rust-native / WGPU-oriented renderer)**:
   * Зафиксирован курс на нативную графическую реализацию в виде Rust/WGPU бэкенда.
   * Написание собственного 3D-движка с нуля не является целью MVP.
   * Заложен отдельный исследовательский этап (*technical research item*) по выбору готового open-source Rust-рендерера/фреймворка. Кандидат **Bevy** рассматривается с осторожностью: допускается только минимальное использование отдельных модулей/подсистем без жесткой привязки архитектуры к полному стек-движку на уровне спецификации.
2. **Прозрачность контейнеров и ослабление фокуса (Focus Attenuation & Semantic Opacity Tiers)**:
   * Проблема перекрытия прозрачных контейнеров в MVP решается через динамическую политику ослабления фокуса (*focus attenuation*).
   * Утверждены стартовые профили прозрачности (*semantic opacity tiers*):
     * **Клик по департаменту**: остальные департаменты текущего слоя получают прозрачность ~50%, департаменты других слоев ~20%.
     * **Клик по шарду**: родительский департамент ~50%, остальные департаменты текущего слоя ~20%, остальные слои ~5%.
   * Сложные алгоритмы сортировки и OIT (Order-Independent Transparency) вынесены в будущие графические оптимизации на случай недостаточности ослабления фокуса и сортировки по глубине.
3. **Сохранение пресетов камер (Camera Presets / Saved Views)**:
   * Пресеты и сохраненные позиции камеры сохраняются вместе с проектом в метаданных (`axicad.project.json` в секции `layout_state` / `viewports`).
   * Данные настройки не сериализуются в биологические TOML-файлы и не влияют на физическую/биологическую модель.
4. **Статус GPU Offscreen Picking Buffer**:
   * Буфер выбора на стороне GPU является исключительной графической оптимизацией (hardware/rendering optimization) и не выступает в качестве источника истины (*source of truth*).
   * Единственным владельцем логики выбора, отсечения, фильтрации и итогового резолвинга целей остаются **Selection Engine** и пространственные политики Raycast.
5. **Границы между 3D HUD и плоским UI (Spatial HUD vs Flat UI)**:
   * Метки, текстовые подписи объектов (сокетов, шардов) и диагностические маркеры ошибок по умолчанию позиционируются в 3D-пространстве сцены как **spatial HUD / world-anchored labels**.
   * Двухмерный (плоский) HTML/Canvas UI зарезервирован для служебных панелей, инспектора свойств, таймлайна симуляции, всплывающих окон и фиксированных поверхностных оверлеев.

---

## 18. Открытые вопросы (Open Decisions)

*Все ключевые архитектурные вопросы графического конвейера рендеринга зафиксированы. Анализ и выбор конкретного open-source Rust-рендерера/фреймворка (включая исследование минимального набора модулей Bevy) переведен в статус активной исследовательской задачи реализационного уровня (implementation research task) и не является блокирующим архитектурным вопросом.*
...
| :--- | :--- | :--- |
| 2026-06-28 | v0.5.0 | Зафиксированы архитектурные решения: утвержден вектор на Rust/WGPU (выбор фреймворка выделен в research task), синхронизирован AABB/grid-aligned рендеринг департаментов в MVP, зафиксирована политика focus attenuation и semantic opacity tiers для прозрачности, определено сохранение пресетов камер в project metadata, подтвержден статус GPU picking buffer как графической оптимизации и определен spatial HUD для 3D-меток. (Закрыты Open Decisions #1-5). |
| 2026-06-28 | v0.4.1 | Восстановлена секция 16.3 Growth & Inference Profile, восстановлена непрерывная нумерация подразделов (16.4 Multi-Viewport Architecture). |
| 2026-06-28 | v0.4.0 | Закреплена архитектура рендеринга нескольких экземпляров вьюпорта (`Multi-Viewport Architecture`) с независимыми камерами, режимами проекции и фокусом (Закрыто Open Decision #35). |
