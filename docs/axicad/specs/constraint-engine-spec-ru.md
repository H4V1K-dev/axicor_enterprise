...
/** Контракт регистрации динамического правила (Dynamic Rule Registry Contract) */
export interface RuleRegistryEntry {
  readonly ruleId: string;                     // Уникальный ключ правила (например, 'RULE-GEO-001')
  readonly ruleVersion: string;                // Семантическая версия правила ('1.0.0')
  readonly ruleProfileId: string;              // Группа правил ('biological' | 'layout' | 'abi')
  readonly introducedInSchemaVersion: string;  // Версия схемы TOML, с которой правило активно
  readonly deprecatedIn?: string;              // Версия схемы, в которой правило устарело
  readonly scope: ConstraintScopeType;        // Область применимости
  readonly defaultSeverity: ConstraintSeverity;// Стандартная критичность
  readonly applicableEntityTypes: string[];   // Применимые типы сущностей
  readonly evaluator: (ctx: ConstraintContext) => ConstraintViolation[];
}
...
| `CE-BIO-005` | `AXI-SCHEMA-004` | `error` | Нарушение биологических соотношений параметров (длина спайка меньше рефрактерности). |
...
| `CE-GEO-010` | `AXI-EDITOR-003` | `error` | Жесткое пространственное пересечение (hard composition collision) департаментов или шардов на сцене. |
| `CE-DRT-011` | `AXI-BAKER-004` | `warning` | Геометрия сокетов изменилась, ручной тракт или сэмплы устарели (stale preview). |
...
## 12. Зафиксированные архитектурные решения (Resolved Architecture Decisions)

1. **Граница между Validation и Constraint Engine (Resolved)**: Constraint Engine служит чистым вычислительным бэкендом (rule evaluation backend). Validation Engine является верхнеуровневой оболочкой, управляющей UX, severity, блокировками UI-операций и сценариями исправления (Mini Repair Mode).
2. **Rust-Portable Core (Resolved)**: Ядро движка разрабатывается как переносимый Rust-модуль (с компиляцией в WASM/JS для браузера), обеспечивая детерминизм между CLI Baker и редактором.
3. **Динамический реестр правил (Resolved)**: Движок оперирует динамическим реестром (`Dynamic Rule Registry`), позволяющим подключать правила по единому контракту без переписывания ядра.
4. **Версионирование правил (Resolved)**: Введена каноническая система версионирования (`ruleId`, `ruleVersion`, `ruleProfileId`, `introducedInSchemaVersion`, `deprecatedIn`) для поддержки профилей совместимости старых проектов.
5. **Политика блокировки коллизий (Resolved)**: Коллизии Composition (пересечения шардов и департаментов) признаны жесткими коллизиями (`hard collisions`), блокирующими команды Apply/commit и запуск Baker. Прочие типы коллизий регулируются severity конкретных правил.
6. **Кэширование результатов в Store (Resolved)**: Актуальный снимок и кэш диагностик `ConstraintViolation` сохраняется непосредственно в реактивном дереве Editor Store, обеспечивая мгновенный отклик UI. Движок пересчитывает нарушения инкрементально.
...
| :--- | :--- | :--- |
| 2026-06-28 | v0.4.0 | Разграничена ответственность с Validation Engine, утвержден Rust-portable core, динамический Rule Registry с версионированием (`ruleVersion`), жесткая блокировка Apply при коллизиях Composition (`AXI-EDITOR-003`) и реактивное кэширование диагностик в Store. Заменен legacy код на `AXI-SCHEMA-004`. Закрыты Open Decisions #21-26. |
| 2026-06-27 | v0.3.0 | Финальная полировка: save-project-json исключен из BlockingOperation, уточнен incremental full validation для JSON проекта, убран legacy код AXI-VALID-004, исправлены "граничные кубы" на боксы, уточнен scope для project metadata, исправлены опечатки в changelog. |
| 2026-06-27 | v0.2.0 | Доработан контракт ConstraintResult (удален timestamp), ослаблена связь с диагностиками, внедрен BlockingOperation, уточнены профили, обновлены proposed коды ошибок и относительные ссылки. |
| 2026-06-27 | v0.1.0 | Создан первый драфт спецификации Constraint Engine для AxiCAD. Описаны структура, области действия, профили, категории правил, детерминизм и открытые вопросы. |
