...
| Поле | Тип данных | Обязательность | Описание |
|---|---|---|---|
| `id` | String | Required *(Target extension)* | Стабильный уникальный идентификатор соединения (`stable connection id/ref`). Первичный механизм связывания для геометрии трактов. *(Требует обновления Rust Serde схемы)* |
...
> [!NOTE]
> **Совместимость схемы связей (Connections Schema Compatibility)**:
> В текущей/legacy реализации Rust Serde до обновления парсеров поле `id` может отсутствовать или быть несовместимым, однако **целевая каноническая схема** требует обязательного наличия `id` как основного первичного связующего ключа (`primary binding`). Строки `from` и `to` хранят человекочитаемые пути эндпоинтов (`human-readable endpoint paths`).
...
> [!NOTE]
> **Архитектурное решение по `typeId` и внешним пресетам**:
> - Явное каноническое поле `typeId` в TOML не требуется и отсутствует. `Type_ID` для Baker ABI рассчитывается строго по индексу позиции элемента в массиве `[[neuron_types]]` (от `0` до `15`). В UI поле `typeId` может существовать исключительно как опциональный псевдоним (`syntactic sugar`).
> - Внешняя межпроектная библиотека нейронных профилей (1800+ типов) хранится на диске во внешней папке вне TOML-структуры проекта. При добавлении типа из библиотеки в шард его параметры копируются в массив `[[neuron_types]]` данного `shard.toml`.
...
| Дата | Изменение |
|------|-----------|
| 2026-06-28 | Зафиксировано обязательное наличие (`Required Target extension`) поля `id` в таблицах `[[connections]]` как primary binding для геометрии трактов. Исправлена опечатка в описании хранилища пресетов. |
| 2026-06-28 | Зафиксировано отсутствие канонического `typeId` в TOML (индекс Baker ABI вычисляется по порядку 0..15) и правила импорта из внешней библиотеки нейронных пресетов вне проекта. (Закрыты Open Decisions #15, 17). |
| 2026-06-28 | Добавлена спецификация опционального поля `id` в таблицы `[[connections]]` для междепартаментных и внутридепартаментных связей как предложенное расширение схемы (Target extension) для устойчивой привязки геометрии трактов. |
