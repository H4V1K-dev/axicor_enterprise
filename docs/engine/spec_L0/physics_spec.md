...
В процессе архитектурного анализа физического ядра выявлены следующие противоречия и открытые вопросы, требующие утверждения главным архитектором:
...
2. **Граница Active Tail Hit (`< prop` vs `<= prop`)**:
   ...
   - *Вопрос*: Фиксируем ли мы строго строгое неравенство `< prop` (где длина хвоста равна ровно `prop` сегментам)?
...
3. **Противоречие Spatial Cooling в GSOP**:
   ...
   - *Вопрос*: Подтверждает ли архитектор окончательное удаление Spatial Cooling из целевой математики `AxiEngine`?
