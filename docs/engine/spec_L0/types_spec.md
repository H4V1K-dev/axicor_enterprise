...
- **Правило применения**: Разрешено использовать **исключительно вне горячего цикла** (например, инициализация, Ночная Фаза, AOT-сборка baker, проверка спонтанного спайка), если иное явно не зафиксировано спецификацией конкретного compute-ядра.
...
4. **Внешняя зависимость `wyhash` vs Inline Реализация**:
   - *Решение в спеке*: Зафиксирован курс на 0 внешних зависимостей для RNG/хеширования внутри `types` (только `bytemuck` и dev-dependency `static_assertions`). Требуется финальное утверждение в спецификации.
