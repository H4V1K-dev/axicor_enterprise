...
#### Канонический Пустой Таргет и Ловушка Нулевого Индекса (Zero-Index Trap)

- **Каноническое пустое состояние памяти (`None`)**: В памяти значение `PackedTarget(0)` (заполненный нулями массив `calloc`/`Zeroable`) обозначает отсутствие связи (`None`).
...
5. **Распиновка `SomaFlags`**:
   - *Решение в спеке*: Зафиксирован канонический макет `SomaFlags`: Bit 0 = Spiking, Bits 1..3 = Burst_Count, Bits 4..7 = Type_ID. Требуется утвердить эту распиновку для всех compute-ядер.
