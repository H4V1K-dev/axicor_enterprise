...
def main():
    ...
    # Load Logs
    log_256_sanity = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_best_log_256_sanity.json"))
    log_256_learning = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_best_log_256_learning.json"))
    log_512_sanity = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_best_log_512_sanity.json"))
    edges_256 = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_best_edge_log_256.json"))
    summary = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_summary.json"))
    sweep = load_json(os.path.join(artifacts_dir, "plastic_microcircuit_v1_3_sweep_summary.json"))
    ...
    plt.hist(unmatched_deltas_mass, bins=30, alpha=0.5, label=f'Unmatched Control (n={len(unmatched_deltas_mass)})', color='red')
    plt.hist(matched_deltas_mass, bins=30, alpha=0.5, label=f'Matched (n={len(matched_deltas_mass)})', color='green')
    plt.axvline(x=np.mean(unmatched_deltas_mass) if unmatched_deltas_mass else 0.0, color='darkred', linestyle='--', label=f'Mean Unmatched: {np.mean(unmatched_deltas_mass) if unmatched_deltas_mass else 0.0:.1f}')
    plt.axvline(x=np.mean(matched_deltas_mass) if matched_deltas_mass else 0.0, color='darkgreen', linestyle='--', label=f'Mean Matched: {np.mean(matched_deltas_mass) if matched_deltas_mass else 0.0:.1f}')
    ...
    plt.figure(figsize=(10, 5))
    plt.hist(unmatched_deltas_exact, bins=30, alpha=0.5, label=f'Unmatched Control (n={len(unmatched_deltas_exact)})', color='red')
    plt.hist(matched_deltas_exact, bins=30, alpha=0.5, label=f'Matched (n={len(matched_deltas_exact)})', color='green')
    plt.axvline(x=np.mean(unmatched_deltas_exact) if unmatched_deltas_exact else 0.0, color='darkred', linestyle='--', label=f'Mean Unmatched: {np.mean(unmatched_deltas_exact) if unmatched_deltas_exact else 0.0:.4f} uV')
    plt.axvline(x=np.mean(matched_deltas_exact) if matched_deltas_exact else 0.0, color='darkgreen', linestyle='--', label=f'Mean Matched: {np.mean(matched_deltas_exact) if matched_deltas_exact else 0.0:.4f} uV')
    ...
    axes[0].axvline(x=np.mean(l4_l23_unmatched) if l4_l23_unmatched else 0.0, color='darkorange', linestyle='--', label=f'Mean Unmatched: {np.mean(l4_l23_unmatched) if l4_l23_unmatched else 0.0:.1f}')
    axes[0].axvline(x=np.mean(l4_l23_matched) if l4_l23_matched else 0.0, color='darkcyan', linestyle='--', label=f'Mean Matched: {np.mean(l4_l23_matched) if l4_l23_matched else 0.0:.1f}')
    ...
    axes[1].axvline(x=np.mean(l4_l5_unmatched) if l4_l5_unmatched else 0.0, color='darkorange', linestyle='--', label=f'Mean Unmatched: {np.mean(l4_l5_unmatched) if l4_l5_unmatched else 0.0:.1f}')
    axes[1].axvline(x=np.mean(l4_l5_matched) if l4_l5_matched else 0.0, color='darkcyan', linestyle='--', label=f'Mean Matched: {np.mean(l4_l5_matched) if l4_l5_matched else 0.0:.1f}')
    ...
    corr_total = l256['corr_total']
    uncorr_total = l256['uncorr_total']

    mean_l4_l23_matched_mass = np.mean(l4_l23_matched) if l4_l23_matched else 0.0
    mean_l4_l23_unmatched_mass = np.mean(l4_l23_unmatched) if l4_l23_unmatched else 0.0
    mean_l4_l5_matched_mass = np.mean(l4_l5_matched) if l4_l5_matched else 0.0
    mean_l4_l5_unmatched_mass = np.mean(l4_l5_unmatched) if l4_l5_unmatched else 0.0
    ...
    control_ok = corr_total > 0 and uncorr_total > 0
    mean_corr_pos = mean_corr_m > 0.0 and mean_corr_e > 0.0
    magnitude_selective_ok = mean_corr_m > mean_uncorr_m and mean_corr_e > mean_uncorr_e
    ratio_ok = corr_pos_ratio > uncorr_pos_ratio
    selective_ok = mean_corr_pos and magnitude_selective_ok
    ...
    if not control_ok:
        verdict = "FAIL / invalid control"
    elif not selective_ok:
        verdict = "PARTIAL / nonselective potentiation"
    else:
        gate_failures = []
        if not phys_ok:
            gate_failures.append("activity gate failed")
        if not ratio_ok:
            gate_failures.append("positive-ratio tie")
        if mean_corr_v == 0:
            gate_failures.append("mass-domain only")

        if gate_failures:
            verdict = "PARTIAL / " + " / ".join(gate_failures)
        elif (invar_dale == "PASS" and invar_sign == "PASS" and invar_bounds == "PASS" and selective_ok and ratio_ok and phys_ok):
            verdict = "PASS"
        else:
            verdict = "PARTIAL PASS"

    # Report Compile
    report_md = f"""# Plastic Microcircuit v1.3 Control-Preserving Potentiation Report

Status: {"completed / pass" if verdict == "PASS" else f"completed / {verdict.lower()}"}
...
>   - N=256 learning: L4=**{r4_256_l:.2f} Hz**, L23=**{r23_256_l:.2f} Hz**, L5=**{r5_256_l:.2f} Hz**.
>   - N=512 sanity: L4=**{r4_512_s:.2f} Hz**, L23=**{r23_512_s:.2f} Hz**, L5=**{r5_512_s:.2f} Hz**.
...
>   - Matched delta mass растет сильнее unmatched: **{mean_corr_m:.1f} > {mean_uncorr_m:.1f}**.
> - **Pathway Selection**: positive-ratio gate не закрыт: matched **{corr_pos_ratio * 100.0:.2f}%**, unmatched **{uncorr_pos_ratio * 100.0:.2f}%**. Unmatched связи тоже растут, но слабее matched.
...
| **Control Validity** | Matched/Unmatched count > 0 | matched={corr_total}, unmatched={uncorr_total} (ratio={corr_total/max(uncorr_total, 1):.1f}) | - | **{"PASS" if control_ok else "FAIL"}** |
| **Moderate Activity** | L4 (3-25Hz), L23 (3-35Hz), L5 (1-15Hz) | L4={r4_256_l:.2f}Hz, L23={r23_256_l:.2f}Hz, L5={r5_256_l:.2f}Hz | L4={r4_512_s:.2f}Hz, L23={r23_512_s:.2f}Hz, L5={r5_512_s:.2f}Hz | **{"PASS" if phys_ok else "FAIL"}** |
| **Selective Potentiation** | matched > unmatched delta & matched_delta > 0 | matched: +{mean_corr_e:.4f} uV, unmatched: {mean_uncorr_e:.4f} uV | - | **{"PASS" if selective_ok else "FAIL"}** |
| **Pathway Positive Ratio** | Matched positive ratio > unmatched | matched={corr_pos_ratio * 100.0:.2f}%, unmatched={uncorr_pos_ratio * 100.0:.2f}% | - | **{"PASS" if ratio_ok else "FAIL"}** |

---

## Параметры победителя (Winner Parameters)

- `fatigue_capacity` = **{wp['fatigue_cap']}**
- `gsop_potentiation` = **{wp['gsop_pot']}**
- `gsop_depression` = **{wp['gsop_dep']}**
- `virt_w` = **{wp['virt_w']}**
- `inh_l23_l4` = **{wp['inh']}**
- `structured_p` = **{wp['structured_p']}**
...
|---|---|---|---|---|---|---|---|
| **Matched** | {corr_total} | {mean_corr_m:.1f} | {mean_corr_e:.4f} | {mean_corr_v:.4f} | {corr_pos_ratio * 100.0:.1f}% | {sum(1 for e in v_to_l4 if e['is_matched'] and e['delta_mass'] == 0) / max(corr_total, 1) * 100.0:.1f}% | {sum(1 for e in v_to_l4 if e['is_matched'] and e['delta_mass'] < 0) / max(corr_total, 1) * 100.0:.1f}% |
| **Unmatched** | {uncorr_total} | {mean_uncorr_m:.1f} | {mean_uncorr_e:.4f} | {mean_uncorr_v:.4f} | {uncorr_pos_ratio * 100.0:.1f}% | {sum(1 for e in v_to_l4 if not e['is_matched'] and e['delta_mass'] == 0) / max(uncorr_total, 1) * 100.0:.1f}% | {sum(1 for e in v_to_l4 if not e['is_matched'] and e['delta_mass'] < 0) / max(uncorr_total, 1) * 100.0:.1f}% |
...
"""
    ...
    with open(os.path.join(report_dir, "plastic_microcircuit_v1_3_control_preserving_potentiation.md"), "w", encoding="utf-8") as f:
        f.write(report_md)

    # README.md
    readme_md = f"""# Research Archive: Plastic Microcircuit v1.3 Control-Preserving Potentiation
...
- Доказано relative matched bias: matched растет сильнее unmatched (mean delta mass: {mean_corr_m:.1f} vs {mean_uncorr_m:.1f}; exact charge: +{mean_corr_e:.4f} uV vs +{mean_uncorr_e:.4f} uV).
...
1. **Virtual->L4 Selective Potentiation**: matched mean {mean_corr_e:.4f} uV vs unmatched {mean_uncorr_e:.4f} uV.
2. **Physiology Status**: partial; N=256 learning L4={r4_256_l:.2f} Hz ниже gate.
