...
| **Описание** | Оркестратор компиляции Ahead-of-Time (AOT Compile Orchestrator) для AxiEngine. В рамках Stage A крейт принимает конфигурацию шарда, seed и voxel size, запускает пайплайн генерации локальной топологии (`topology`) и осуществляет сборку локального single-shard набора артефактов в памяти. Stage B добавляет возможность упаковки этих сгенерированных артефактов в ROM-контейнер `.axic` через `vfs`. **Не-цель (non-goal)**: компилятор `baker` ориентирован строго на AOT-операции. Выполнение динамического планирования и sprouting passes Ночной Фазы в процессе симуляции не входит в зону ответственности `baker`, делегируясь `weaver-daemon` и `topology`. |
