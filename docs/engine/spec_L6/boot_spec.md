...
*Примечание о путях (Paths Working Copy)*: Координатная сетка `paths_blob` извлекается и валидируется на этапе загрузки, но не загружается в вычислительное ядро (INV-BOOT-A06). Во время Ночной Фазы этот буфер `paths_blob` передается рантаймом в качестве изменяемой рабочей копии (Host Working copy / Mutable Working Seed) для алгоритмов синаптогенеза. Исходный файл `.axic` никогда не перезаписывается.
