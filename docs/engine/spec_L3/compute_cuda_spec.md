...
2. **Защита от Гонок Данных во VRAM**: Запись спайков in `axon_heads` ядрами GPU исключает гонки данных во VRAM за счет использования дисюнктной карты `soma_to_axon` или буфера накопления во VRAM.
...
21. **Проверка контракта обслуживания бэкендом (`test_cuda_maintenance_contract_stub`)**: Проверка наличия методов обслуживания в трейте `ComputeBackend` и возврата `UnsupportedFeature` (или успешного завершения при наличии реализации).
...
5. **[RESOLVED] Поддержка экспорта и импорта обслуживания (isomorphic DMA)**:
   - *Решение*: Описан контракт обслуживания шарда через DMA (H2D/D2H) для isomorphic parity с CPU-бэкендом. Night policy (sort_and_prune) явно запрещена внутри CUDA-ядер бэкенда.
