...
| `baker` | [baker_spec.md](spec_L4/baker_spec.md) | **Approved v2.2 / Ready for Implementation (Stage B)** | AOT компилятор и локальный упаковщик `.axic` через `vfs` без контроля политик загрузки boot. |
