...
fn run_night_phase_dormant_bank_v0_7() {
    ...
    {
        let mut runner = SimulationRunner::new(&flat_axons, &mut flat_synapses, &variants, &soma_variants, seed_val);
    ...
    for syn in &initial_synapses_day1 {
        ...
        *initial_target_proj_counts.entry((syn.target_soma_id, proj)).or_insert(0) += 1;
    }

    // Determine absolute floor budget limit (weight < 1498 * 65536)
    let budget_total = initial_synapses_day1.iter().filter(|s| s.weight.abs() < 1498 << 16).count();
    println!("Budget total (based on absolute floor < 1498): {}", budget_total);
    ...
    println!("Pre-night 1 matched bias: {:.4}", pre_night_results.matched_bias);
    ...
    for syn in &initial_synapses_day1 {
        ...
        let tv = soma_variants[syn.target_soma_id as usize];
        let proj = get_projection_type(sv, tv);
        proj_pre_hits.entry(proj.clone()).or_default().push(syn.pre_hits);
    ...
    for syn in &initial_synapses_day1 {
        ...
        if coactivity_ratio_pct >= 400 || syn.long_trace >= 20 {
            *target_proj_useful_count.entry((syn.target_soma_id, proj)).or_insert(0) += 1;
        }
    }
    ...
    for (idx, syn) in initial_synapses_day1.iter().enumerate() {
        ...
        let count_useful = *target_proj_useful_count.get(&(syn.target_soma_id, proj.clone())).unwrap_or(&0);
    ...
    for policy in &policies {
        ...
        match *policy {
            ...
            "hard_delete_trace_aware" => {
                ...
                let victims_indices: HashSet<usize> = sorted_scores.iter().take(budget_total).map(|&(idx, _)| idx).collect();
                let mut survivors = Vec::new();
        ...
        println!("  Day 2 Active / Dormant / Deleted: {} / {} / {}", active_day2, dormant_day2, deleted_day2);
        ...
        {
            let mut runner = SimulationRunner::new(&flat_axons, &mut current_synapses, &variants, &soma_variants, seed_val);
        ...
        assert_eq!(day2_results.dale_violations, 0, "Dale violations detected under {} on Day 2!", policy);
        assert_eq!(day2_results.dense_violations, 0, "Dense target violations detected under {} on Day 2!", policy);
        assert_eq!(day2_results.duplicate_violations, 0, "Duplicate violations detected under {} on Day 2!", policy);
        assert_eq!(day2_results.runaway_ticks, 0, "Runaway ticks detected under {} on Day 2!", policy);
        assert!(day2_results.silence_ticks < max_ticks, "Complete silence collapse under {} on Day 2!", policy);
        ...
        let full_cohort_bias_day2 = compute_full_cohort_metrics(&initial_synapses_day1, &current_synapses, &soma_variants);
        ...
        if *policy == "dormant_trace_aware_with_return" {
            ...
            for syn in &*runner.flat_synapses {
                ...
                let tv = soma_variants[syn.target_soma_id as usize];
                let proj = get_projection_type(sv, tv);
                *target_proj_counts.entry((syn.target_soma_id, proj)).or_insert(0) += 1;
            }
            ...
            for (d_idx, ds) in dormant_synapses.iter().enumerate() {
                ...
                let initial_count = *initial_target_proj_counts.get(&(ds.target_soma_id, ds.projection_class.clone())).unwrap_or(&0);
                let current_proj_count = *target_proj_counts.get(&(ds.target_soma_id, ds.projection_class.clone())).unwrap_or(&0);
                ...
                if trace_ok { trace_ok_count += 1; }
                if slot_ok { slot_ok_count += 1; }
                if diversity_ok { diversity_ok_count += 1; }
                if activity_ok { activity_ok_count += 1; }
                ...
                if trace_ok && slot_ok && diversity_ok && activity_ok {
                    ...
                    *target_proj_counts.entry((ds.target_soma_id, ds.projection_class.clone())).or_insert(0) += 1;
                }
            }

            println!("  Reactivation blocker breakdown (out of {}):", dormant_synapses.len());
        ...
        println!("  Day 4 Active / Dormant / Deleted: {} / {} / {}", active_day4, dormant_day4, deleted_day4);
        ...
        assert_eq!(day4_results.dale_violations, 0, "Dale violations detected under {} on Day 4!", policy);
        assert_eq!(day4_results.dense_violations, 0, "Dense target violations detected under {} on Day 4!", policy);
        assert_eq!(day4_results.duplicate_violations, 0, "Duplicate violations detected under {} on Day 4!", policy);
        assert_eq!(day4_results.runaway_ticks, 0, "Runaway ticks detected under {} on Day 4!", policy);
        assert!(day4_results.silence_ticks < max_ticks, "Complete silence collapse under {} on Day 4!", policy);
        ...
        let full_cohort_bias_day4 = compute_full_cohort_metrics(&initial_synapses_day1, runner.flat_synapses, &soma_variants);
        ...
        // Dormant metrics
        let dormant_matched = dormant_synapses.iter().filter(|ds| ds.source_soma_id < 48 && ds.target_soma_id >= 128 && ds.target_soma_id < 176).count();
        let dormant_high_trace = dormant_synapses.iter().filter(|ds| ds.long_trace >= 20).count();

        println!("  Matched full-cohort retention Day2 / Day4: {:.4} / {:.4}", matched_retention_day2_full, matched_retention_day4_full);
        println!("  Survivor retention Day2 / Day4: {:.4} / {:.4}", matched_retention_day2_surv, matched_retention_day4_surv);
        println!("  Silence ticks Day2 / Day4: {} / {}", day2_results.silence_ticks, day4_results.silence_ticks);
        println!("  Dormant Protected Matched count: {}", dormant_matched);
        println!("  Dormant Protected High Long Trace count: {}", dormant_high_trace);
