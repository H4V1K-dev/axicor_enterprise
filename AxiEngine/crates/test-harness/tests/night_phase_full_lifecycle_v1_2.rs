...
#[derive(Debug, Clone, serde::Serialize)]
struct PolicyLifecycleDataV12 {
    policy: String,
    topology_mode: String,
    cycles: Vec<CycleMetricsV12>,
}

#[derive(Debug, Clone, serde::Serialize)]
struct PlottingDataV12 {
    policies: Vec<PolicyLifecycleDataV12>,
}
...
#[test]
fn run_night_phase_full_lifecycle_v1_2() {
    println!("=== Starting Night Phase Full Lifecycle v1.2 ===");
    ...
    let policies = [
        "passive_night_baseline",
        "dormant_reactivation_only",
        "sprouting_only",
        "full_lifecycle",
    ];
    ...
    for policy_name in &policies {
        ...
        for cycle in 1..=total_cycles {
            ...
            if *policy_name != "passive_night_baseline" {
                ...
                for syn in active_synapses {
                    ...
                    if syn.weight.abs() < 500 << 16 {
                        pruned_count += 1;
                        if *policy_name == "dormant_reactivation_only"
                            || *policy_name == "full_lifecycle"
                        {
                            dormant_added += 1;
                ...
                // b) Dormant Age Increment & Expiration Pass (Eviction after 3 dormant cycles)
                if *policy_name == "dormant_reactivation_only" || *policy_name == "full_lifecycle" {
                    let k_long = 7;
                    ...
                    reactivated_count = reactivated_indices.len();
                ...
                if *policy_name == "sprouting_only" || *policy_name == "full_lifecycle" {
                    ...
                    for &(target_id, _pressure) in &under_recruited_targets {
                        ...
                        let mut rng_seed =
                            deterministic_rng(seed_val, target_id as u32, cycle * 100);
            ...
            } else if *policy_name == "full_lifecycle" && reactivated_count == 0 && cycle == 5 {
                "PARTIAL (Sprouting Stable, Dormant Reactivation 0)".to_string()
            } else {
                "STABLE_SPROUTING".to_string()
            };

            // 4. Calculate Cohort Metrics for Rare Context B
            ...
            let n_active = active_synapses.len().max(1) as f64;
            let long_sat_count = active_synapses
                .iter()
                .filter(|s| s.long_trace >= 20)
                .count();
            let short_sat_count = active_synapses
                .iter()
                .filter(|s| s.short_trace >= 100)
                .count();
            ...
            cycle_metrics_list.push(CycleMetricsV12 {
                cycle,
                has_context_b,
                ...
                long_trace_saturation_fraction: long_sat_count as f64 / n_active,
                short_trace_saturation_fraction: short_sat_count as f64 / n_active,
        ...
        policy_results.push(PolicyLifecycleDataV12 {
    ...
    let archive_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_full_lifecycle_v1_2/artifacts");
