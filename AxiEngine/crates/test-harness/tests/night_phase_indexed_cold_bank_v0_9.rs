...
fn run_night_phase_indexed_cold_bank_v0_9() {
    ...
    let policies = vec![
        ...
        "dormant_scan_context_reactivation",
        "dormant_indexed_any_day",
        "dormant_indexed_bucketed",
        "dormant_indexed_bucketed_plus_trace",
    ];

    let mut plotting_policies = Vec::new();
    let mut oracle_reactivated_set: HashSet<(u32, u32, u32)> = HashSet::new();
    ...
    for policy in &policies {
        ...
        let mut indexed_evidence = IndexedEvidence::default();
        {
            ...
            let dormant_ref = if *policy == "dormant_scan_context_reactivation" {
                Some(dormant_synapses.as_mut_slice())
            } else {
                None
            };
            let ev_ref = if *policy == "dormant_scan_context_reactivation"
                || *policy == "hard_delete_trace_aware"
            {
                None
            } else {
            ...
            runner.run_day(max_ticks, true, true, dormant_ref, ev_ref);
        }
        ...
        let mut blockers = BlockerBreakdownV09 {
            trace_ok: 0,
            context_ok: 0,
            slot_ok: 0,
            diversity_ok: 0,
        ...
        let summary_sizes = SummarySizesV09 {
            hit_set_size: indexed_evidence.source_segment_hit_set.len(),
            spike_set_size: indexed_evidence.target_spike_set.len(),
            hit_bucket_count: indexed_evidence.source_segment_buckets.len(),
            spike_bucket_count: indexed_evidence.target_spike_buckets.len(),
        };
        ...
        let mut current_reactivated_set = HashSet::new();

        let mut indexed_checks = 0;

        let requires_reactivation = *policy != "hard_delete_trace_aware";
        ...
        if requires_reactivation {
            ...
            for (d_idx, ds) in dormant_synapses.iter().enumerate() {
                ...
                let context_ok = match *policy {
                    "dormant_scan_context_reactivation" => {
                        ds.dormant_context_hits >= 3
                            || (ds.short_trace > 0 && ds.dormant_context_hits > 0)
                    }
                    "dormant_indexed_any_day" => {
                        indexed_checks += 2;
                        indexed_evidence
                            .source_segment_hit_set
                            .contains(&(ds.source_soma_id, ds.flat_segment_idx))
                            && indexed_evidence
                                .target_spike_set
                                .contains(&ds.target_soma_id)
                    }
                    "dormant_indexed_bucketed" => {
                        let mut ok = false;
                    ...
                    "dormant_indexed_bucketed_plus_trace" => {
                        ...
                        ok && (ds.short_trace > 0 || ds.long_trace > 0)
                    }
                    _ => false,
                };
            ...
            for (d_idx, ds) in dormant_synapses.iter().enumerate() {
                if reactivated_indices.contains(&d_idx) {
                    ...
                    current_reactivated_set.insert((
                        ds.source_soma_id,
                        ds.flat_segment_idx,
                        ds.target_soma_id,
                    ));
        ...
        if *policy == "dormant_scan_context_reactivation" {
            oracle_reactivated_set = current_reactivated_set.clone();
        }
        ...
        // Precision / Recall / Jaccard vs Oracle Scan
        let intersection = oracle_reactivated_set
            .intersection(&current_reactivated_set)
            .count();
        let union = oracle_reactivated_set
            .union(&current_reactivated_set)
            .count();
        let (precision, recall, jaccard) = if *policy == "hard_delete_trace_aware" {
            (0.0, 0.0, 0.0)
        } else if *policy == "dormant_scan_context_reactivation" {
            (1.0, 1.0, 1.0)
        } else {
            let p_val = if current_reactivated_set.is_empty() {
                1.0
            } else {
                intersection as f64 / current_reactivated_set.len() as f64
            };
            let r_val = if oracle_reactivated_set.is_empty() {
                1.0
            } else {
                intersection as f64 / oracle_reactivated_set.len() as f64
            };
            let j_val = if union == 0 {
                1.0
            } else {
        ...
        let (scan_checks_total, indexed_checks_total, reduction_factor) =
            if *policy == "dormant_scan_context_reactivation" {
                let total_scan = 864 * max_ticks;
                (total_scan, 0, 1.0)
            } else if *policy == "hard_delete_trace_aware" {
                (0, 0, 1.0)
            } else {
                let total_scan = 864 * max_ticks;
                let total_indexed = indexed_evidence.day_inserts + indexed_checks;
                let factor = if total_indexed > 0 {
                    total_scan as f64 / total_indexed as f64
                } else {
                    total_scan as f64
                };
                (total_scan, total_indexed, factor)
            };

        println!(
            "  Precision / Recall / Jaccard vs Oracle: {:.4} / {:.4} / {:.4}",
            precision, recall, jaccard
        );
        println!(
            "  Cost scan vs indexed checks (Reduction): {} vs {} ({:.2}x)",
            scan_checks_total, indexed_checks_total, reduction_factor
        );
        println!(
            "  Summary sizes: hits={} spikes={} hit_buckets={} spike_buckets={}",
            summary_sizes.hit_set_size,
            summary_sizes.spike_set_size,
            summary_sizes.hit_bucket_count,
            summary_sizes.spike_bucket_count
        );

        plotting_policies.push(PolicyPlotDataV09 {
            ...
            summary_sizes,
            cost: CostEstimateV09 {
                scan_checks: scan_checks_total,
                indexed_checks: indexed_checks_total,
                reduction_factor,
            },
        });
    }
    ...
    let dest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_indexed_cold_bank_v0_9/artifacts/plot_data.json");
    if let Some(parent) = dest_path.parent() {
        std::fs::create_dir_all(parent).unwrap();
    }
    std::fs::write(&dest_path, json_content).unwrap();
    println!("Saved v0.9 plotting data to {:?}", dest_path);
}
