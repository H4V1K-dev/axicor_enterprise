...
struct PairHistoryStats {
    ...
    decayed_to_zero_count: usize,
    positive_events_total: usize,
    negative_events_total: usize,
}

#[derive(Debug, Clone, serde::Serialize)]
struct CycleMetricsV15 {
    cycle: usize,
    branch: String,
    has_context_b: bool,
    active_count: usize,
    ...
    // Negative control false recovery
    false_recovery_rate: f64,
...
#[derive(Debug, Clone, serde::Serialize)]
struct PolicyBranchDataV15 {
    policy: String,
    branch: String,
    cycles: Vec<CycleMetricsV15>,
}

#[derive(Debug, Clone, serde::Serialize)]
struct PlottingDataV15 {
    experiments: Vec<PolicyBranchDataV15>,
}
...
fn compute_ph_stats(pair_history: &HashMap<(u32, u32), PairHistory>) -> PairHistoryStats {
    if pair_history.is_empty() {
        return PairHistoryStats::default();
    }
    let mut masses: Vec<u32> = pair_history.values().map(|p| p.mass).collect();
    masses.sort();
    let n = masses.len();
    let sum: u64 = masses.iter().map(|&m| m as u64).sum();
    let mean = sum as f64 / n as f64;

    let p50 = masses[n / 2];
    let p90 = masses[(n as f64 * 0.9) as usize % n];
    let max = *masses.iter().max().unwrap_or(&0);

    let sat_count = masses.iter().filter(|&&m| m >= 1000).count();
    let dec_zero = masses.iter().filter(|&&m| m == 0).count();

    let pos_total: usize = pair_history
        .values()
        .map(|p| p.positive_events as usize)
        .sum();
    let neg_total: usize = pair_history
        .values()
        .map(|p| p.negative_events as usize)
        .sum();
    ...
    PairHistoryStats {
        ...
        mass_p90: p90,
        mass_max: max,
        saturation_fraction: sat_count as f64 / n as f64,
        decayed_to_zero_count: dec_zero,
        positive_events_total: pos_total,
        negative_events_total: neg_total,
    }
}

#[test]
fn run_night_phase_pair_history_prior_v1_5() {
    println!("=== Starting Night Phase Pair-History Prior Probe v1.5 ===");
    ...
    let policies = [
        "baseline_fresh_sprout",
        "pair_history_init_weight",
        "pair_history_init_trace",
        "pair_history_weight_plus_trace",
        "pair_history_overstrong_stress",
    ];

    let branches = ["returned_branch", "absent_branch"];
    ...
    for policy_name in &policies {
        for branch_name in &branches {
            println!(
                "=== Evaluating Policy: {} | Branch: {} ===",
                policy_name, branch_name
            );
            ...
            let mut pair_history: HashMap<(u32, u32), PairHistory> = HashMap::new();
            let mut cycle_metrics_list = Vec::new();

            for cycle in 1..=total_cycles {
                let has_context_b = match *branch_name {
                    "returned_branch" => cycle == 1 || cycle == 2 || cycle == 5,
                    "absent_branch" => cycle == 1 || cycle == 2, // Cycle 5 Context B remains ABSENT!
                    _ => unreachable!(),
                };

                println!(
                    "  [Policy: {} | Branch: {}] Cycle {} / {} (Context B: {})",
                    policy_name, branch_name, cycle, total_cycles, has_context_b
                );
                ...
                // 2. Night Phase Updates & Pair History Tracking

                // Gain / Penalty parameters
                let mass_gain = if *policy_name == "pair_history_overstrong_stress" {
                    50
                } else {
                    10
                };
                ...
                // Update pair_history for active synapses that survived
                for syn in &active_synapses {
                    if syn.weight.abs() >= 500 << 16 {
                        let ph = pair_history
                            .entry((syn.source_soma_id, syn.target_soma_id))
                            .or_default();
                        ph.mass = (ph.mass + mass_gain).min(1000);
                        ph.positive_events = ph.positive_events.saturating_add(1);
                        ph.last_seen_cycle = cycle as u16;
                    }
                }

                // Pruning Pass into Dormant Bank
                ...
                for syn in active_synapses {
                    ...
                    if syn.weight.abs() < 500 << 16 {
                        pruned_count += 1;
                        // Apply disuse penalty to pair_history
                        if let Some(ph) =
                            pair_history.get_mut(&(syn.source_soma_id, syn.target_soma_id))
                        {
                            ph.mass = ph.mass.saturating_sub(mass_penalty);
                        }
                ...
                for ds in dormant_synapses {
                    if ds.dormant_age > 3 {
                        if let Some(ph) =
                            pair_history.get_mut(&(ds.source_soma_id, ds.target_soma_id))
                        {
                            ph.mass = ph.mass.saturating_sub(dead_penalty);
                            ph.negative_events = ph.negative_events.saturating_add(1);
                        }
                        dead_synapses.push((
                            ds.source_soma_id,
                            ds.flat_segment_idx,
                ...
                // Global Cyclic Pair History Mass Decay (`mass -= mass >> 3`)
                for ph in pair_history.values_mut() {
                    ph.mass = ph.mass.saturating_sub(ph.mass >> 3);
                }

                // Reactivation Pass (Baseline indexed reactivation)
                ...
                for &r_idx in &reactivated_indices {
                    ...
                    // Apply pair-history bias during reactivation if policy dictates
                    let ph_mass = pair_history
                        .get(&(ds.source_soma_id, ds.target_soma_id))
                        .map(|p| p.mass)
                        .unwrap_or(0);

                    let (w_init, s_tr, l_tr) = match *policy_name {
                        "pair_history_init_weight" => {
                            let w_bias = (ph_mass as i32 * 50) << 16;
                            let sign = if ds.weight < 0 { -1 } else { 1 };
                            let base_w = ds.weight.abs();
                            let combined_w = (base_w + w_bias).min(3000 << 16);
                            (sign * combined_w, ds.short_trace, ds.long_trace)
                        }
                        "pair_history_init_trace" => {
                            let s_bias = (ph_mass * 5) as u16;
                            let l_bias = (ph_mass * 2) as u16;
                            (
                                ds.weight,
                                (ds.short_trace + s_bias).min(500),
                                (ds.long_trace + l_bias).min(30),
                            )
                        }
                        "pair_history_weight_plus_trace" => {
                            let w_bias = (ph_mass as i32 * 50) << 16;
                        ...
                        "pair_history_overstrong_stress" => {
                            let w_bias = (ph_mass as i32 * 200) << 16;
                            let sign = if ds.weight < 0 { -1 } else { 1 };
                            let base_w = ds.weight.abs();
                            let combined_w = (base_w + w_bias).min(6000 << 16);
                            let s_bias = (ph_mass * 20) as u16;
                            let l_bias = (ph_mass * 10) as u16;
                            (
                                sign * combined_w,
                                (ds.short_trace + s_bias).min(1000),
                                (ds.long_trace + l_bias).min(100),
                            )
                        }
                        _ => (ds.weight, ds.short_trace, ds.long_trace),
                    };
                ...
                // Stochastic Geometry Sprouting into Freed Headroom with Pair-History Bias
                ...
                for &(target_id, _pressure) in &under_recruited_targets {
                        ...
                        && !eligible_candidates.is_empty()
                    {
                        ...
                        for cand in &eligible_candidates {
                            ...
                            // Pair history mass bonus
                            let ph_mass = pair_history
                                .get(&(cand.syn.source_soma_id, target_id as u32))
                                .map(|p| p.mass)
                                .unwrap_or(0);
                            let ph_bonus = 1.0 + (ph_mass as f32 * 0.005); // Up to 6.0x probability bonus

                            let w = (-beta * cand.orig_syn.distance_sq).exp()
                                * diversity_bonus
                                * ph_bonus;
                            weights.push(w);
                            sum_w += w;
                        ...
                        if current_pair_count < 2 {
                            ...
                            let (w_init, s_tr, l_tr) = match *policy_name {
                                "pair_history_init_weight" => {
                                    ...
                                    let combined = ((1500 << 16) + w_bias).min(3000 << 16);
                                    (sign * combined, 50, 0)
                                }
                                "pair_history_init_trace" => {
                                    let s_bias = (ph_mass * 5) as u16;
                                    let l_bias = (ph_mass * 2) as u16;
                                    (base_w << 16, (50 + s_bias).min(500), l_bias.min(30))
                                }
                                "pair_history_weight_plus_trace" => {
                                    let w_bias = (ph_mass as i32 * 50) << 16;
                                    let sign = if is_inhibitory { -1 } else { 1 };
                                    ...
                                    let s_bias = (ph_mass * 5) as u16;
                                    let l_bias = (ph_mass * 2) as u16;
                                    (sign * combined, (50 + s_bias).min(500), l_bias.min(30))
                                }
                                "pair_history_overstrong_stress" => {
                                    let w_bias = (ph_mass as i32 * 200) << 16;
                                    let sign = if is_inhibitory { -1 } else { 1 };
                                    let combined = ((1500 << 16) + w_bias).min(6000 << 16);
                                    let s_bias = (ph_mass * 20) as u16;
                                    let l_bias = (ph_mass * 10) as u16;
                                    (sign * combined, (50 + s_bias).min(1000), l_bias.min(100))
                                }
                                _ => (base_w << 16, 50, 0),
                            };
                ...
                // Safety Gate Checks
                let safety_metrics =
                    compute_metrics(&active_synapses, &soma_variants, &day_metrics);
                if *policy_name != "pair_history_overstrong_stress" {
                    assert_eq!(
                        safety_metrics.dale_violations, 0,
                        "Dale violations in policy {} branch {} cycle {}!",
                        policy_name, branch_name, cycle
                    );
                    assert_eq!(
                        safety_metrics.dense_violations, 0,
                        "Dense violations in policy {} branch {} cycle {}!",
                        policy_name, branch_name, cycle
                    );
                    assert_eq!(
                        safety_metrics.duplicate_violations, 0,
                        "Duplicate violations in policy {} branch {} cycle {}!",
                        policy_name, branch_name, cycle
                    );
                    assert_eq!(
                        safety_metrics.runaway_ticks, 0,
                        "Runaway ticks in policy {} branch {} cycle {}!",
                        policy_name, branch_name, cycle
                    );
                }
                ...
                // False recovery rate calculation in absent_branch
                let false_recovery_rate = if *branch_name == "absent_branch" && cycle == 5 {
                    let rare_new_total = rare_reactivated_count + rare_sprouted_new_count;
                    if sprouted_count + reactivated_count > 0 {
                        rare_new_total as f64 / (sprouted_count + reactivated_count) as f64
                    } else {
                        0.0
                    }
                } else {
                ...
                } else if *branch_name == "absent_branch"
                    && (rare_reactivated_count > 50 || rare_sprouted_new_count > 500)
                {
                    "HALLUCINATION (False Recovery High)".to_string()
                } else if rare_sprouted_new_count > 800 || rare_reactivated_count > 100 {
                    "STRONG_SUCCESS (Rare Recovery High)".to_string()
                } else {
                    "DIAGNOSTIC (Normal Dynamic)".to_string()
                };
                ...
                let ph_stats = compute_ph_stats(&pair_history);

                cycle_metrics_list.push(CycleMetricsV15 {
                    cycle,
            ...
            experiment_results.push(PolicyBranchDataV15 {
    ...
    let archive_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_pair_history_prior_v1_5/artifacts");
