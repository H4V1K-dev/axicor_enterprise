...
struct IndexedEvidence {
    soma_spike_buckets: HashSet<(u32, usize)>,
...
#[derive(Debug, Clone, serde::Serialize)]
struct PolicyLifecycleDataV14 {
    policy: String,
    topology_mode: String,
    cycles: Vec<CycleMetricsV14>,
}

#[derive(Debug, Clone, serde::Serialize)]
struct PlottingDataV14 {
    policies: Vec<PolicyLifecycleDataV14>,
}
...
impl<'a> SimulationRunner<'a> {
    ...
    fn run_day(
        ...
        for tick in 0..ticks {
            ...
            // Record ALL soma spiking events into soma_spike_buckets
            if record_trace {
                for &soma_id in &spiking_somas {
                    if let Some(ref mut ev) = indexed_evidence {
                        ev.soma_spike_buckets.insert((soma_id, tick / 8));
                    }
                }
...
#[test]
fn run_night_phase_soma_indexed_reactivation_v1_4() {
    println!("=== Starting Night Phase Soma-Indexed Dormant Reactivation v1.4 ===");
    ...
    let policies = [
        "segment_index_baseline_v1_3",
        "soma_bucket_cofire",
        "soma_bucket_plus_trace",
        "soma_bucket_plus_trace_plus_slot_pressure",
    ];

    let mut policy_results = Vec::new();

    for policy_name in &policies {
        println!("=== Evaluating Reactivation Policy: {} ===", policy_name);
        ...
        for cycle in 1..=total_cycles {
            ...
            // Eviction Pass (evict if dormant_age > 3)
            ...
            let max_target_slot = if *policy_name == "soma_bucket_plus_trace_plus_slot_pressure" {
                64
            } else {
                96
            };
            ...
            for (d_idx, ds) in dormant_synapses.iter().enumerate() {
                ...
                // Check evidence depending on Policy
                let pass_evidence = match *policy_name {
                    "segment_index_baseline_v1_3" => {
                    ...
                    "soma_bucket_cofire" => {
                        let mut cofire_ok = false;
                        for b in 0..(max_ticks / 8) {
                            if indexed_evidence
                                .soma_spike_buckets
                                .contains(&(ds.source_soma_id, b))
                                && (indexed_evidence
                                    .soma_spike_buckets
                                    .contains(&(ds.target_soma_id, b))
                                    || indexed_evidence
                ...
                if !pass_evidence {
                    react_blocker.react_evidence_failed += 1;
                    continue;
                }

                let target_count = target_active_counts[ds.target_soma_id as usize];
                let slot_ok = target_count < max_target_slot;
                if !slot_ok {
                    react_blocker.react_slot_failed += 1;
                    continue;
                }
            ...
            } else if rare_reactivated_count > 100 {
                "STRONG_SUCCESS (Rare Memory Reactivated)".to_string()
            } else if rare_reactivated_count > 10 {
                "PARTIAL_SUCCESS (Reactivation Increased)".to_string()
            } else {
                "DIAGNOSTIC (Reactivation Low)".to_string()
            };
            ...
            cycle_metrics_list.push(CycleMetricsV14 {
                cycle,
                has_context_b,
        ...
        policy_results.push(PolicyLifecycleDataV14 {
    ...
    let archive_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_soma_indexed_reactivation_v1_4/artifacts");
