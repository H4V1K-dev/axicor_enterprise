...
#[derive(Debug, Clone, serde::Serialize)]
struct PlottingDataV16 {
    cycles: Vec<CycleMetricsV16>,
}
...
fn compute_safety_metrics(
    ...
    // Load Whitelist for validation
    let shard_config = build_shard_config();
    let neuron_types = &shard_config.neuron_types;
    ...
    for target_id in 0..n {
        ...
        for syn in incoming {
            *source_counts.entry(syn.source_soma_id).or_insert(0) += 1;
            ...
            let tgt_type = &neuron_types[soma_variants[target_id] as usize];
            let src_type = &neuron_types[soma_variants[syn.source_soma_id as usize] as usize];
...
#[test]
fn run_night_phase_mvp_consolidation_v1_6() {
    println!("=== Starting Night Phase MVP Consolidation Probe v1.6 ===");
    ...
    for cycle in 1..=total_cycles {
        // Stimulus schedule: Cycles 1-2 (Mixed), Cycles 3-6 (Sparse), Cycles 7-10 (Mixed Returns)
        ...
        if cycle > 4 {
            // After warmup, make sure we do not grow monotonically without bounds (should be bounded by cap)
            assert!(
        ...
        cycle_metrics_list.push(CycleMetricsV16 {
            cycle,
            ...
            sprouted_count,
            reactivated_count: 0, // Reactivation not required/implemented in v1.6
    ...
    // Write metric results to artifacts
    let archive_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_mvp_consolidation_v1_6/artifacts");
