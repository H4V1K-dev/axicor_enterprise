...
struct PolicyPlotDataV10 {
    ...
    matched_bias_retention: f64,
    monopoly_top_5pct_share: f64,
    sprouted_coords: Vec<SproutCoordinate>,
}

#[derive(Debug, Clone, serde::Serialize)]
struct PlottingDataV10 {
    policies: Vec<PolicyPlotDataV10>,
}
...
impl<'a> SimulationRunner<'a> {
    ...
    fn run_day(
        ...
        for tick in 0..max_ticks {
            ...
            // Oracle side-channel pre-trace updates
            ...
            // Production-like indexed evidence pre-synaptic segment hits side-output
            if let Some(ref mut ev) = indexed_evidence {
                let bucket_id = tick / 8;
                for source_id in 0..n {
                    for (seg_idx, &is_active) in self.active_segments[source_id].iter().enumerate()
                    {
                        if is_active {
                            ev.source_segment_hit_set
                                .insert((source_id as u32, seg_idx as u32));
            ...
            // Oracle side-channel target-spike updates
            if let Some(ref mut dormant_list) = dormant_synapses {
                for ds in dormant_list.iter_mut() {
            ...
            // Production-like indexed evidence target spikes side-output
            if let Some(ref mut ev) = indexed_evidence {
                let bucket_id = tick / 8;
                for i in 0..n {
                    if spikes_this_tick[i] {
                        ev.target_spike_set.insert(i as u32);
                        ev.target_spike_buckets.insert((i as u32, bucket_id));
                        ev.day_inserts += 2;
                    }
                }
            }
            ...
            // Oracle side-channel evidence timer decay
            if let Some(ref mut dormant_list) = dormant_synapses {
                for ds in dormant_list.iter_mut() {
...
#[derive(serde::Serialize)]
struct SummarySizesV09 {
    hit_set_size: usize,
    spike_set_size: usize,
    hit_bucket_count: usize,
    spike_bucket_count: usize,
}

#[derive(serde::Serialize)]
struct CostEstimateV09 {
    scan_checks: usize,
    indexed_checks: usize,
    reduction_factor: f64,
}

#[derive(serde::Serialize)]
struct PolicyPlotDataV09 {
    name: String,
    active_day1: usize,
    active_day2: usize,
    active_day4: usize,
    ...
    recall: f64,
    jaccard: f64,
    blockers: BlockerBreakdownV09,
    summary_sizes: SummarySizesV09,
    cost: CostEstimateV09,
}

#[derive(serde::Serialize)]
struct PlottingDataV09 {
    policies: Vec<PolicyPlotDataV09>,
}

#[test]
fn run_night_phase_indexed_cold_bank_v0_9() {
    println!("=== Starting Night Phase Indexed Cold Bank Evidence v0.9 ===");
    ...
    let policies = vec![
        ...
        "active_source_greedy_sprouting",
        "under_recruited_target_sprouting",
        "under_recruited_plus_projection_diversity",
        "under_recruited_plus_diversity_plus_stochastic_geometry",
    ];

    let mut plotting_policies = Vec::new();

    for policy in &policies {
        println!("\n--- Testing Sprouting Policy: {} ---", policy);

        let mut current_synapses = initial_synapses_day1.clone();
        let mut dormant_synapses: Vec<DormantSynapse> = Vec::new();

        // 1. Demote to dormant under pruning policy (trace-aware Night 1)
        {
        ...
        // Compact synapses after Demotion
        {
        ...
        let day2_results: MetricResults;
        {
            ...
            day2_metrics = runner.run_day(max_ticks, false, true, None, None);
        ...
        let mut day3_spike_counts = vec![0u32; n_somas];
        {
            ...
            // Compile day summaries in indexed_evidence
            runner.run_day(max_ticks, true, true, None, Some(&mut indexed_evidence));
            for i in 0..n_somas {
                day3_spike_counts[i] = runner.somas[i].spike_count;
            }
        }

        // Night 2: trace merge, dormant decay, reactivation, and sprouting pass
        ...
        // 2. Reactivation pass using bucketed_plus_trace (standard for v1.0)
        ...
        for (d_idx, ds) in dormant_synapses.iter().enumerate() {
            ...
            // Compute mean projection count on this target for diversity check
            let proj_classes = [
                "Virtual->L4",
                "L4->L23",
                "L4->L5",
            ...
            let mean_proj = if proj_counts_on_target.is_empty() {
                0
            } else {
                proj_counts_on_target.iter().sum::<usize>() / proj_counts_on_target.len()
            };
        ...
        // Sort: most under-recruited first (lowest pressure)
        under_recruited_targets.sort_by_key(|&(_, p)| p);

        // Build active & dormant sets to identify potential candidates
        let mut active_pairs: HashSet<(u32, u32, u32)> = runner
            .flat_synapses
            .iter()
        ...
        if *policy != "no_sprouting_baseline" {
            println!("  Sprouting new synapses for policy {}...", policy);
            ...
            for &(target_id, _pressure) in &under_recruited_targets {
                ...
                // Sort candidates depending on policy
                match *policy {
                    "active_source_greedy_sprouting" => {
                        target_candidates.sort_by(|a, b| {
                            let act_a = day3_spike_counts[a.syn.source_soma_id as usize];
                            let act_b = day3_spike_counts[b.syn.source_soma_id as usize];
                            act_b.cmp(&act_a)
                        });
                    }
                    "under_recruited_target_sprouting"
                ...
                if *policy == "under_recruited_plus_diversity_plus_stochastic_geometry" {
                    let mut rng_seed = deterministic_rng(seed_val, target_id as u32, 999);
                    let mut step_count = 0;

                    // Compute mean projection count on this target for diversity bonus
                    let proj_classes = [
                        "Virtual->L4",
                        "L4->L23",
                        "L4->L5",
                    ...
                    // Keep the same per source-target cap as the other policies.
                    // If dense C17 already saturates this cap, stochastic sprouting
                    // must report no structural headroom instead of weakening the
                    // invariant to force new connections.
                        ...
                        && !eligible_candidates.is_empty()
                    {
                        ...
                        if current_pair_count < 2 {
                            ...
                            runner.flat_synapses.push(sprouted_syn);
                    ...
                    for cand in target_candidates {
                        ...
                        if *policy == "under_recruited_plus_projection_diversity" {
                            // Compute mean projection count on this target
                            let proj_classes = [
                                "Virtual->L4",
                                "L4->L23",
                                "L4->L5",
                            ...
                            let mean_proj = if proj_counts_on_target.is_empty() {
                                ...
                                proj_counts_on_target.iter().sum::<usize>()
                                    / proj_counts_on_target.len()
                            };
                            let current_proj_count = *active_proj_counts
                                .get(&(target_id as u32, proj.clone()))
                                .unwrap_or(&0);
                            // Only allow sprouting into projections at or below mean
        ...
        // 4. Re-compact active synapses (important to prevent dense violations)
        {
        ...
        let new_synapse_count = sprouted_synapses.len();
        let dead_day4 =
            (initial_synapses_day1.len() + new_synapse_count) - active_day4 - dormant_day4;

        println!(
            "  Day 4 Active / Dormant / Sprouted / Deleted: {} / {} / {} / {}",
            active_day4, dormant_day4, new_synapse_count, dead_day4
        );

        // Day 4 Replay (without learning)
        println!("  Running Day 4 Replay (learning disabled)...");
        let day4_metrics = runner.run_day(max_ticks, false, true, None, None);
        let day4_results = compute_metrics(runner.flat_synapses, &soma_variants, &day4_metrics);

        // Get day 4 spike counts
        let mut day4_spike_counts = vec![0u32; n_somas];
        for i in 0..n_somas {
            day4_spike_counts[i] = runner.somas[i].spike_count;
        }
        ...
        for name in &expected_proj_pairs {
            sprouted_by_proj.insert(name.to_string(), 0);
        }
        ...
        let mut fan_in_distribution = vec![0; n_somas];
        for syn in runner.flat_synapses.iter() {
            fan_in_distribution[syn.target_soma_id as usize] += 1;
        }

        let gini_coefficient = compute_gini(&fan_in_distribution);
        ...
        for syn in runner.flat_synapses.iter() {
            ...
            let tv = soma_variants[syn.target_soma_id as usize];
            let proj = get_projection_type(sv, tv);
            active_projections.insert(proj);
        }
        let projection_coverage = active_projections.len() as f64 / 7.0;
        ...
        for t_idx in 0..n_somas {
            ...
            let pressure = (day3_spike_counts[t_idx] as i32) - target_rate;
            if pressure < 0 {
                under_recruited_somas.push(t_idx);
            }
        }
        ...
        let matched_bias_retention = if pre_night_results.matched_bias.abs() > 1e-5 {
            day4_results.matched_bias / pre_night_results.matched_bias
        } else {
            0.0
        };

        let mut sprouted_by_target = vec![0usize; n_somas];
        for syn in &sprouted_synapses {
            sprouted_by_target[syn.target_soma_id as usize] += 1;
        }
        let mut sorted_sprouted_by_target = sprouted_by_target.clone();
        sorted_sprouted_by_target.sort_by_key(|&v| std::cmp::Reverse(v));
        let top_k = (n_somas as f64 * 0.05).round() as usize;
        let top_k = top_k.max(1);
        let top_sum: usize = sorted_sprouted_by_target.iter().take(top_k).sum();
        let total_sprouted: usize = sorted_sprouted_by_target.iter().sum();
        let monopoly_top_5pct_share = if total_sprouted > 0 {
            top_sum as f64 / total_sprouted as f64
        } else {
            0.0
        };

        let mut sprouted_coords = Vec::new();
        if *policy == "under_recruited_plus_diversity_plus_stochastic_geometry" {
            for syn in &sprouted_synapses {
                let mut found_coord = None;
                for cand in &candidate_pool {
                    if cand.syn.source_soma_id == syn.source_soma_id
                        && cand.syn.flat_segment_idx == syn.flat_segment_idx
                        && cand.syn.target_soma_id == syn.target_soma_id
                    {
                        let axon = &axons[syn.source_soma_id as usize];
                        let seg = &axon.branches[cand.orig_syn.branch_id as usize]
                            [cand.orig_syn.segment_offset as usize - 1];
                        found_coord = Some(SproutCoordinate {
                            x: seg.x,
                            y: seg.y,
                            z: seg.z,
                            src: syn.source_soma_id,
                            tgt: syn.target_soma_id,
                            proj: get_projection_type(
                                soma_variants[syn.source_soma_id as usize],
                                soma_variants[syn.target_soma_id as usize],
                            ),
                        });
                        break;
                    }
                }
                if let Some(coord) = found_coord {
                    sprouted_coords.push(coord);
                }
            }
        }

        plotting_policies.push(PolicyPlotDataV10 {
            name: policy.to_string(),
            active_count: active_day4,
            dormant_count: dormant_day4,
            dead_count: dead_day4,
            sprouted_count: new_synapse_count,
            sprouted_by_proj,
            fan_in_distribution,
            gini_coefficient,
            projection_coverage,
            ...
            matched_bias_retention,
            monopoly_top_5pct_share,
            sprouted_coords,
        });
    }

    // Save plot data
    let plot_data = PlottingDataV10 {
    ...
    let dest_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../../docs/engine/research/archive/2026-07-06_night_phase_sprouting_diversity_v1_0/artifacts/plot_data.json");
    if let Some(parent) = dest_path.parent() {
        std::fs::create_dir_all(parent).unwrap();
    }
    std::fs::write(&dest_path, json_content).unwrap();
    println!("Saved v1.0 plotting data to {:?}", dest_path);
}
